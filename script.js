...
// Función auxiliar para desinfectar entradas de texto contra XSS Stored/Reflected
...
function actualizarInterfacesProveedores() {
    document.querySelectorAll('.select-proveedores').forEach(sel => {
        ...
        cacheProveedores.forEach(p => {
            const safeNombre = sanitizarHTML(p.nombre);
            sel.innerHTML += `<option value="${safeNombre}">${safeNombre}</option>`;
        });[cite: 4]
    ...
    if (lista) {
        ...
        cacheProveedores.forEach(p => {
            ...
            const chips = prods.map(pr => `<span class="bg-slate-800 text-slate-300 text-xs px-2 py-0.5 rounded border border-slate-700">${sanitizarHTML(pr)}</span>`).join(' ');[cite: 4]
            lista.innerHTML += `<div class="p-3 bg-slate-950 rounded-lg border border-slate-800"><p class="font-bold text-sm text-white">${sanitizarHTML(p.nombre)}</p><div class="flex flex-wrap gap-1 mt-2">${chips || '<span class="text-xs text-slate-600 italic">Sin mercancía</span>'}</div></div>`;[cite: 4]
        });
    }
}
...
function filtrarProductosPorProveedor(prov, idDestino) {
    ...
    if (item && Array.isArray(item.productos) && item.productos.length > 0) {
        ...
        item.productos.forEach(pr => {
            const safeProd = sanitizarHTML(pr);
            dest.innerHTML += `<option value="${safeProd}">${safeProd}</option>`;
        });[cite: 4]
    ...
    data.detallesLotes.forEach(l => {
        ...
        tbody.insertAdjacentHTML("beforeend", `
            <tr class="hover:bg-slate-900/50 border-b border-slate-800">
                <td class="px-6 py-4 font-mono text-xs font-bold text-blue-400">${sanitizarHTML(l.idLote)}</td>
                <td class="px-6 py-4 font-medium">${sanitizarHTML(l.proveedor)}</td>
                <td class="px-6 py-4">${sanitizarHTML(l.producto)}</td>
...
function agregarFilaMercanciaPrepago() {
    ...
    productos.forEach(pr => {
        const safePr = sanitizarHTML(pr);
        opcionesHtml += `<option value="${safePr}">${safePr}</option>`;
    });[cite: 4]
...
function imprimirReciboPrepago() {
    const prov = sanitizarHTML(document.getElementById("pre-proveedor").value);[cite: 4]
    const lote = sanitizarHTML(document.getElementById("pre-lote-sugerido").innerText);[cite: 4]
    const tasa = parseFloat(document.getElementById("pre-tasa").value) || 1;[cite: 4]
    const fecha = sanitizarHTML(document.getElementById("pre-fecha").value);[cite: 4]
    
    let htmlItems = "";[cite: 4]
    document.querySelectorAll(".item-fila-prepago").forEach(fila => {
        const prod = sanitizarHTML(fila.querySelector(".item-producto").value);[cite: 4]
        const cant = parseFloat(fila.querySelector(".item-cantidad").value) || 0;[cite: 4]
...
/**
 * Función que ejecuta la lógica del motor FIFO y actualiza KPIs y tablas dinámicamente
 */
function actualizarTablaRecepcionCascada() {
    ...
    lotesDisponibles.forEach(lote => {
        ...
        row.innerHTML = `
            <td class="px-3 py-2 font-mono text-xs font-bold text-blue-400">${sanitizarHTML(lote.idLote)}</td>
    ...
    // Asignar el valor calculado del motor FIFO al KPI
    const elKpiBsActual = document.getElementById("rec-kpi-bs-actual");
    if (elKpiBsActual) {
        if ('value' in elKpiBsActual) {
            elKpiBsActual.value = formatearMonto(sumBsActual);
        } else {
            elKpiBsActual.innerText = formatearMonto(sumBsActual);
        }
    }
...
function imprimirComprobanteRecepcion() {
    const prov = sanitizarHTML(document.getElementById("rec-proveedor") ? document.getElementById("rec-proveedor").value : "");[cite: 4]
    const prod = sanitizarHTML(document.getElementById("rec-producto") ? document.getElementById("rec-producto").value : "");[cite: 4]
    const cant = parseFloat(document.getElementById("rec-cantidad") ? document.getElementById("rec-cantidad").value : 0) || 0;[cite: 4]
    const fecha = sanitizarHTML(document.getElementById("rec-fecha") ? document.getElementById("rec-fecha").value : new Date().toISOString().split('T')[0]);[cite: 4]
    ...
    const factura = sanitizarHTML(numFacturaInput ? numFacturaInput.value : "N/A");[cite: 4]
    ...
    if (tbody) {
        ...
        trs.forEach(tr => {
            ...
            if (cols.length >= 8) {
                const idLote = sanitizarHTML(cols[0].innerText);[cite: 4]
...
function procesarEnvioProveedor(e) {
    ...
    window.respuestaProveedorGoogle = function(res) {
        if (res && res.status === "success") {[cite: 4]
            alert(`🤝 Proveedor "${sanitizarHTML(nombre)}" registrado exitosamente.`);[cite: 4]
    ...
    datosFiltrados.forEach(h => {
        tbody.insertAdjacentHTML("beforeend", `
            <tr class="hover:bg-slate-900/50 border-b border-slate-800 text-xs">
                <td class="px-4 py-3 font-mono text-slate-400">${sanitizarHTML(h.fecha || '-')}</td>
                <td class="px-4 py-3 font-mono font-bold text-blue-400">${sanitizarHTML(h.idLote)}</td>
                <td class="px-4 py-3 font-medium text-white">${sanitizarHTML(h.proveedor)}</td>
                <td class="px-4 py-3 text-slate-300">${sanitizarHTML(h.producto)}</td>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Ajustar escuchadores según eventos del formulario
    const recCantidad = document.getElementById("rec-cantidad");
    if (recCantidad) recCantidad.addEventListener("input", actualizarTablaRecepcionCascada);

    const recTasa = document.getElementById("rec-tasa");
    if (recTasa) recTasa.addEventListener("input", actualizarTablaRecepcionCascada);

    const recProv = document.getElementById("rec-proveedor");
    if (recProv) recProv.addEventListener("change", function() { actualizarFlujoProveedorRecepcion(this.value); });
