...
/**
 * Recalcula la tabla FIFO, asigna 'rec-monto-bs' al KPI 'rec-kpi-bs-actual'
 * y distribuye proporcionalmente dicho monto en la columna 'TOTAL HOY (BS)'
 */
function actualizarTablaRecepcionCascada() {
    // 1. Obtener y actualizar el KPI 'Valor Hoy (Bs)' desde el input de la factura
    const inputMontoBs = document.getElementById("rec-monto-bs");
    const elKpiBsActual = document.getElementById("rec-kpi-bs-actual");
    const montoFacturaBs = parseFloat(inputMontoBs ? inputMontoBs.value : 0) || 0;

    if (elKpiBsActual) {
        if ('value' in elKpiBsActual) {
            elKpiBsActual.value = montoFacturaBs;
        } else {
            elKpiBsActual.innerText = typeof formatearMonto === 'function' ? formatearMonto(montoFacturaBs) : montoFacturaBs;
        }
    }
    ...
    if (!prov || !prod || !cacheUltimosDatos || !cacheUltimosDatos.data || !cacheUltimosDatos.data.detallesLotes) {
        ...
        if (typeof resetearKPIsRecepcion === 'function') resetearKPIsRecepcion();[cite: 4]
        return;
    }
    ...
    // Primer recorrido: Calcular el total USD consumido en la cola FIFO
    lotesDisponibles.forEach(lote => {
        let cantTomada = 0;[cite: 4]
        if (remanentePorDespachar > 0) {
            cantTomada = Math.min(remanentePorDespachar, lote.cantDisponible);[cite: 4]
            remanentePorDespachar -= cantTomada;[cite: 4]
        }
        sumUsdOriginal += cantTomada * (lote.costoUsd || 0);[cite: 4]
    });

    // Resetear remanente para renderizar las filas
    remanentePorDespachar = cantRecepcion;[cite: 4]
    
    // Calcular tasa implícita actual basada en la factura
    const tasaImplicitaFactura = sumUsdOriginal > 0 ? (montoFacturaBs / sumUsdOriginal) : 0;
    let sumBsActual = 0;

    // Segundo recorrido: Renderizado de la tabla con los montos actualizados
    lotesDisponibles.forEach(lote => {
        ...
        // Total Hoy (Bs) calculado usando la tasa de la factura actual
        const totalBsActualTomado = totalUsdTomado * tasaImplicitaFactura;
